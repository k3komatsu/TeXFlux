...
- `texflux_module_system_design.md` — 本設計の出発点となった方針メモ。以降「方針メモ」と呼ぶ。
- `AGENTS.md` — 実装上の境界条件。

本書が方針メモと食い違う場合は、**本書が優先する**（差分は §1 に列挙した）。
...
| # | 項目 | 決定 | 理由 |
| --- | --- | --- | --- |
| D1 | フラグ束縛の構文 | **`!import{quiz.tfx}(answers=on)`**（後置 `(...)` リスト）を採用。方針メモ §27.2 の `!import[...]{...}` 形は不採用。 | 方針メモ §4/§28 の多数派表記に合わせる。実装上の影響は §4 の方式で `!` セグメント末尾に限定できる。 |
...
| D6 | モジュールパス | **相対パスのみ**。絶対パスは拒否する。`..` は許可。 | §26 の再利用可能コンポーネントの可搬性。プラットフォーム差の排除。 |
| D7 | `ModuleInstance` のキャッシュ | パース結果（`ModuleSource`）のみキャッシュし、コンパイル結果はキャッシュしない。 | v1 の単純性。方針メモ §5 の「パースはキャッシュしてよい」に合致。 |
...
方針メモ §25 のとおり、2 種類の依存は決して 1 つに統合しない。
...
**同名フラグの暗黙継承は存在しない**（方針メモ §27.2）。`answers=$answers` のように
明示的に転送したときだけ伝播する。
...
`appendix` が off のとき、このファイルは開かれず、存在しなくてもよい（方針メモ §6）。
...
**公開インタフェース**（方針メモ §10）: `.tfxm` が公開するのは**自分が `!defmacro` した
マクロだけ**である。`!macroimport` で得たマクロは私的な実装依存であり、再輸出されない。
`public` / `private` / `export` 構文は導入しない。

**自己完結性検査**（方針メモ §12）: 各マクロモジュールの環境が確定した後、
...
`.tfxm` の循環 import は許容される（方針メモ §16）。以下のアルゴリズムは
...
`import` と `macroimport` の検出を追加する（方針メモ §7）。
...
`core.tfxm` の `!wrapper` は `main.tfx` からは見えない（方針メモ §8, §9）。
...
@{} >> !import{foo.tfx}          % valid（TeX グループで囲む。方針メモ §18）
...
禁止されるのは**マクロテンプレート内に書くこと**だけである（方針メモ §7）。
...
**2 つの独立したモジュールインスタンス**としてコンパイルされる（方針メモ §5）。

### 8.3 循環検出

コンテンツ import の循環はエラーである（方針メモ §15）。判定は
...
- span を書き換えないので、エディタの逆引きは**実際に壊れている行**へ飛ぶ
  （方針メモ §21 の「呼び出し側の `!import` 行へ付け替えない」と同じ思想）。
...
TeXFlux は被 import モジュールの生 TeX を検証しない（方針メモ §17）。
`\newcommand` などの生 TeX マクロは TeXFlux の名前空間システムの外にある（§18）。
...
解決する（方針メモ §19）。親がどこから取り込んだかには依存しない。
...
正準パス identity は次の 4 つに使う（方針メモ §20）。
...
- キャッシュは意味論を変えてはならない（方針メモ §13）。`ModuleInstance` はキャッシュしない（D7）。
...
（`_definition` の `name in builtins` 検査）。これは方針メモ §11 の
「組み込み特殊名との衝突はエラー」という要求と一致する。
...
被 import モジュールの正準 AST ノードは**自分の span を保持する**（方針メモ §21）。
...
- `--flag` は**ルートモジュールにのみ**適用される。被 import モジュールへの伝播は
  `!import{...}($flag)` による明示的な転送だけである（方針メモ §27.2）。
