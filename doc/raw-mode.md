...
`\item Note:` は行末のコロンが構造化構文の候補になってパースエラーになり、`\textbf{Note}:` は
**黙って** `\textbf{Note}{` + 改行 + `}` を出力し、`\emph{x} >> \emph{y}` は**黙って**スタック合成として
読まれる。後半 2 件はエラーにならないので書き手は踏んだことに気づけない。この 1 行を 3 行の領域で
囲むのは重いので、1 行を丸ごと生にする raw escape `!| ` を併せて設けた（§5）。
