...
- **`flags.py`**: `!when` で捨てられる payload の中の領域も、パースは通り、展開も
  正規化もされない。従来どおり。
- **`_writes_own_braces`**（[normalize.py:215-234](../src/texflux/normalize.py#L215-L234)）:
  シーケンス値が `{` 始まり `}` 終わりの生行のみなら EXPLICIT 配置になる既存規則は、
  raw 領域の行にもそのまま適用される。したがって、シーケンス値を raw 領域で書く場合も、
  最初の本文行が `{`、最後の本文行が `}` になれば、既存の明示ブレース検出が働く。
