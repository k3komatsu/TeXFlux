...
`!off` は、必須インライングループ（`{...}`）をちょうど1つと、ブロックペイロードを受け取る。グループの内容は不透明な生 TeX のまま無視し、ペイロードだけを正規化してその位置へ展開する。

~~~text
\fuga >> !off{\foo{a}{b}} >> \hoge
~~~

これは `\fuga >> \hoge` と同じ結果になる。ブロックペイロードは `: |` の suite、または `>>` の残り部分として渡す。シーケンスの suffix（`:`）、グループの欠落・複数指定・括弧種別違い、ペイロードの欠落はバリデーションエラーとなる。

### 11.3 drop

`!drop` はグループを取らず、ブロックペイロードを正準 AST へ変換せずに捨てる。

~~~text
!drop >> \hoge >> \fuga
~~~

この例は TeX の内容を一切出力しない。ブロックペイロードは `: |` の suite、または `>>` の残り部分として渡す。グループ、シーケンスの suffix（`:`）、またはペイロードの欠落はバリデーションエラーとなる。
...
テンプレート専用構文、組み込み special、自分が定義したマクロ、自分が取り込んだマクロの
いずれかでなければならない。呼び出し側の無関係な名前空間が、本来不正なマクロモジュールを
正当化することは決してない。
...
取り込みとローカル定義、予約語や組み込み special 名との衝突のいずれも同じで、
「後の import が勝つ」という規則は存在しない。
...
削除された `!block`、`!arg`、`!body` は互換用エイリアスとしては提供されず、未知の special としてエラーになる。
