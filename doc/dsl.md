...
末尾がコロンのみ（`:`）の行に続く suite は、`-` で明示された値エントリー（value entry）の並び（シーケンス）を表す。構造化コマンドのシーケンススイートには、少なくとも1つのエントリーが必要である。各 `-` が1つのブロック値の開始を示し、同じインデント階層にある次の `-` の直前までがその値に含まれる。したがって値の境界は `-` のみによって決定される。生成される中括弧がその値に密着するか独立行になるかは、値が1行に収まるかどうかで決まる（15章を参照）。
...
| `\TextCA{注意}:`（項目の継続行） | パースエラー（同じ規則だが、診断は「シーケンススイートには `-` エントリーが必要」となり、コロン規則とは結びつかない） | `\TextCA{注意}:{}` |
...
いずれも、宣言済みのフラグ名を指す必須インライングループを1つ以上取る。ペイロードは `: |` ブロックスイート、または `>>` 合成の残り部分のいずれかで書く。シーケンススイート（`:` のみ）、ペイロードの欠落、未宣言のフラグ名、同じフラグの重複指定は、いずれもバリデーションエラーとなる。
