...
構造化構文と認識される位置で、文字どおりの `@` / `!` から始まる生の行を出力したい場合は、`@@` / `!!` と記述してエスケープできる。先頭の1文字だけを取り除き、残りは生の TeX 行として扱う。ブロック内では構造化構文の追加インデント検査より前に処理され、基準位置より余分な字下げを保持する。シーケンスの `-` の直後の payload にも使える（`- !!bar` → `!bar`）。タブ禁止の規則は変わらない。
...
パラメータ名には `[A-Za-z_][A-Za-z0-9_-]*` の文字パターンが使用できる。パラメータ名の重複、予約名（`defmacro`、`param`、`text`、`each`、`flag`、`when`、`unless`）の使用、組み込みの special と同名の使用、標準フロー制御（11.1節）と同名の使用、およびマクロ自体の多重定義は、いずれも定義位置（definition site）を指し示すバリデーションエラーとなる。
