...
生の TeX、`@@` でエスケープした行、`!items` 内の生テキストでは、行末の `>>` は継続を意味しない。グループ内部を改行で分割することもできない。セグメント・グループ・suffix の位置情報は元の物理行を指し、構文 AST では従来と同じ1つの `Stack` になる。
...
また `!items` のエントリ本文は生の項目テキストであり、その内部では条件を含むいかなる `!` 構文も読まれない。リスト全体を条件付きにしたい場合は `!items` ごと包む。
