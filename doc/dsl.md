...
末尾がコロンのみ（`:`）の行に続く suite は、`-` で明示された値エントリー（value entry）の並び（シーケンス）を表す。構造化コマンドのシーケンススイートには、少なくとも1つのエントリーが必要である。各 `-` が1つのブロック値の開始を示し、同じインデント階層にある次の `-` の直前までがその値に含まれる。したがって値の境界は `-` のみによって決定される。生成される中括弧がその値に密着するか独立行になるかは、値が1行に収まるかどうかで決まる（14章を参照）。
...
捨てられるペイロードが満たしていなければならないのは、**条件の解決より前に行われる検査のすべて**である。すなわち、構文として解釈できること、`!flag` がトップレベルの文であること、そして 12 章のスタック形式の規則（`!defmacro` を `>>` のセグメントにしないこと、`!each` が自前の `: |` を持つこと）の3つであり、これがその全部である。
...
## 16. 構文 AST と正準 AST（Syntax AST / Canonical AST）
...
## 17. 位置情報（SourceSpan）、ソースマップ、SyncTeX
