...
```
末尾が完結したコマンドなら suffix は不要です。本文を続ける場合は最後のセグメントに `::`（ブロック）または `:::`（シーケンス）を付けます。

### 3. 🔀 1つのソースから自在に出し分け（`!flag` / `!when`）
...
### 4. 📦 モジュールシステム（`!import` / `!macroimport`）
...
@frame{TeXFlux の魅力}::
    @itemize::
        \item<1-> \textbf{直感的}: インデントでスッキリ記述
        \item<2-> \textbf{安全}: 閉じタグのミスが原理的に発生しない
        \item<3-> \textbf{柔軟}: TeX の数式やマクロは 100\% そのまま
...
> **エンジンについて。** 上の `.tfx` は `dvipdfmx` 指定と和文を含むので、(u)pLaTeX でビルドします。
...
TeXFlux の文法は極めてシンプルです。**「3つの接頭辞」** と **「2つの suite marker」** を知るだけで、すべての機能を使いこなせます。
...
※ その行を**丸ごと**生の TeX にするには `!| `（`!`、`|`、半角スペース1個）と書きます。`@@` / `!!` が先頭の1文字を剥がすのに対し `!| ` はマーカーごと取り除くので、`!| \emph{x} >> \emph{y}` のように**`\` で始まる行にも使えます**（`>>` や `::` / `:::` を持つコマンド行は、これが無いと構造化構文として解釈されてしまいます）。`!|` の直後が半角スペース1個でも行末でもない場合はエラーです（`!|` だけの行は空の生の行になります）。この行は verbatim で、タブも書けますし `!text{...}` も補間されません。生成シーケンスの payload（`- !| \emph{x} >> \emph{y}`）でも使えますが、そこではタブは書けません。`@@` / `!!` と同じく `+` の明示グループには適用されません。長い領域は次の raw mode で囲みます（領域内はタブと `!text{...}` を含む完全な verbatim です）。
...
※ これらの規則は、環境の本文に書く行にも同じように適用されます。たとえば `\item まとめ:` は、単独のコロンが suite marker ではないので常にそのまま生の TeX です（次の行を字下げしても変わりません）。詳細は [doc/dsl.md 8 章](doc/dsl.md) の表を参照してください。

---

### 2. 2つの suite marker（Suite Suffix）

行末のコロンの数で、インデントされた子要素の受け渡し方を指定します。**単独の `:` は suite marker ではありません**。TeX の散文が行末に置く文字なので、TeXFlux は使いません（`\textbf{Note}:` は常にそのまま出力されます）。

| 記法 | モード | 意味 | 主な用途 |
| :---: | :--- | :--- | :--- |
| `::` | **ブロックモード** | 続くインデント全体を **「1つの本文・ブロック」** として渡す | 環境の本文、単一の長大な引数 |
| `:::` | **シーケンスモード** | 各 `-` / `+` エントリーを順に渡す（`-` は生成必須引数、`+` は明示グループ） | 複数引数を持つコマンド・環境 |

> `>>` が `>` を2つ重ねるのと同じで、TeXFlux の演算子はすべて同じ文字の重ね打ちです。1文字の `:` や `>` は常に TeX のものなので、TeX の散文と衝突しません。
...
#### ③ ブロックの前後に何かを置く（`!before` / `!after` / `!around`）
ペイロードの前後に、書いたとおりの TeX を1つずつ置きます。余白の量も命令も
著者が選ぶもので、コンパイラは位置だけを決めます。
```text
...
\vspace{2em}
```
片側だけなら `!before{\vspace{1em}}` / `!after{\vspace{2em}}` を使います。
`!around{A}{B}` は `!before{A} >> !after{B} >>` と同じ結果です。
...
展開後:
```latex
\twoargs{第1引数のテキスト}[第2引数のオプション]
```

`-` の先頭に `{...}` を書いた場合、その中括弧は内容として扱われ、生成された
中括弧がさらに外側に付きます。引数自身の区切り文字を保持したい場合は `+` を使います。
...
`!drop` が捨てたペイロードは正規化も import の解決もされないので、中の `!import` はファイルを開きません。
ただしマクロ展開そのものは行われるため、引数の数が合わないマクロ呼び出しなどはエラーになります。
壊れて通らなくなった箇所を一時的に黙らせたい場合は `!when` / `!unless` を使ってください。

※ ③ と ⑤ の5つ（`!before` / `!after` / `!around` / `!off` / `!drop`）は
**標準フロー制御**と呼ばれ、コンパイラに同梱されたマクロモジュールで定義された
ただのソースマクロです。import は不要で、`.tfx` からも `.tfxm` からも使えます。
同じ名前を `!defmacro` で定義することはできません。
...
文字どおりの marker はテキスト中で `!!text{x}`、行頭では `!!!text{x}` または `!| !text{x}` と書きます。
...
**A. 一切問題ありません。**  
TeXFlux が認識するのは、行頭のプレフィックス（`@`, `!`, `\`）や行末の suite marker（`::` / `:::`）、前後をスペースで区切られた `>>` だけです。数式の中（`$f: X \to Y$` など）やグループ内の文字を勝手にパースして破壊することはありません。**単独のコロンはどこに書かれても構造化トークンではない**ので、`\textbf{Note}:` も `\group_begin:` のような LaTeX3 のコマンドも、そのまま透過されます。

### Q. 生成された `.tex` の改行が、出力に影響することはありませんか？
**A. 1 か所だけあり、著者が制御できます。**  
TeXFlux は生成した中括弧を `{%` で開くので、開き側の改行が TeX に渡ることはありません。
一方、値を閉じる `}` は独立行に置かれるため、その直前の改行が空白トークン 1 個になります。
これが見えるのは `\scalebox` や `\textbf` のように引数が hbox になる場合だけで、段落や
`center` のような垂直文脈では捨てられます。消したい場合は値の最終行を `%` で終えるか、
その引数を `+ {...}` の明示グループで書きます。
...
引数が**照合キー**であるコマンド（`\label`、`\ref`、`\cite`、`\index` など）では、この空白は
余白ではなくキーそのものの違いになります。`\label:::` の `- sec:intro` は `sec:intro ` という
キーを作り、`\ref{sec:intro}` と一致しません。キーやファイル名は生の TeX 行で
`\label{sec:intro}` と書くのが最も素直です。

なお、ブロック値の中に置いた空行は TeX にとって `\par` です。`\section` のように `\long`
でないコマンドの引数に空行を入れると `Runaway argument` になります。空になりうる引数を
マクロから渡す場合は `!param` ではなく `!text` を使います（`\wrap{!text{body}}`）。詳細は
[doc/dsl.md 15.1 節](doc/dsl.md) を参照してください。
...
そして、もし必要であれば、`@frame::` や `@center >> @{\small}::` などの構造化記法を少しずつ導入していくことが可能です。
...
- [basic.tfx](examples/basic.tfx) : 基本的な環境、生の TeX、`@itemize`、オーバーレイ
