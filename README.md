...
- **軽量マークダウンツールの限界**: Marp や Slidev、Pandoc などの Markdown ベースのスライドツールは手軽ですが、**「学会指定の Beamer テーマ」「高度な数式・TikZ 図」「オーバーレイアニメーション（`<1->`）」「自作 TeX マクロ」** を思い通りに使うのは困難です。
...
> **「既存の学会テンプレートや発表スライドのクオリティを一切落とさずに、タイピング量とストレスだけを劇的に減らしたい」**  
...
- **無効化されたフラグのブロックは展開も検査も行われない**ため、書きかけでコンパイルが通らないスライドを一時的にスキップするのにも最適です。
...
| `!` | **TeXFlux 特殊機能** | `!items:`, `!when`, `!unless`, `!flag`, `!defmacro`, `!vpad` |
...
**A. はい、100% 使えます。**  
TeXFlux は TeX のプリアンブルやマクロ定義、パッケージ読み込み（`\usepackage{...}`）をそのまま透過します。お手持ちの学会テンプレートやカスタムスタイルを一切変更することなく利用可能です。
...
**A. 簡単に部分導入できます。**  
スライド全体を一括で書き換える必要はありません。新しく追加する章やスライドだけを `chapter1.tfx` として作成し、コンパイルした `chapter1.tex` をメインの `.tex` から `\input{chapter1.tex}` で読み込むだけで、今日から段階的に利用を始められます。
