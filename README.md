...
※ その行を**丸ごと**生の TeX にするには `!| `（`!`、`|`、半角スペース1個）と書きます。`@@` / `!!` が先頭の1文字を剥がすのに対し `!| ` はマーカーごと取り除くので、`!| \item Note:` のように**`\` で始まる行にも使えます**（末尾に `:` や `>>` を持つコマンド行は、これが無いと構造化構文として解釈されてしまいます）。`!|` の直後が半角スペース1個でも行末でもない場合はエラーです（`!|` だけの行は空の生の行になります）。この行は verbatim で、タブも書けますし `!text{...}` も補間されません。生成シーケンスの payload（`- !| \item Note:`）でも使えますが、そこではタブは書けません。`@@` / `!!` と同じく `+` の明示グループには適用されません。長い領域は次の raw mode で囲みます（領域内はタブと `!text{...}` を含む完全な verbatim です）。
...
※ これらの規則は、環境の本文に書く行にも同じように適用されます。たとえば `\item まとめ:` は行末のコロンが予約されているためパースエラーになります（`!| \item まとめ:` または `\item まとめ:{}` と書きます）。詳細は [doc/dsl.md 8 章](doc/dsl.md) の表を参照してください。

---

### 2. 2つのブロック形式（Suite Suffix）

行末のコロンの数で、インデントされた子要素の受け渡し方を指定します。
...
**A. 一切問題ありません。**  
TeXFlux が認識するのは、行頭のプレフィックス（`@`, `!`, `\`）や行末の suite suffix（`:` / `::`）、前後をスペースで区切られた `>>` だけです。数式の中（`$f: X \to Y$` など）やグループ内の文字を勝手にパースして破壊することはありません。
