...
※ その行を**丸ごと**生の TeX にするには `!| `（`!`、`|`、半角スペース1個）と書きます。`@@` / `!!` が先頭の1文字を剥がすのに対し `!| ` はマーカーごと取り除くので、`!| \emph{x} >> \emph{y}` のように**`\` で始まる行にも使えます**（`>>` を持つコマンド行や、末尾の `:` の直後にインデントしたブロックが続くコマンド行は、これが無いと構造化構文として解釈されてしまいます）。`!|` の直後が半角スペース1個でも行末でもない場合はエラーです（`!|` だけの行は空の生の行になります）。この行は verbatim で、タブも書けますし `!text{...}` も補間されません。生成シーケンスの payload（`- !| \emph{x} >> \emph{y}`）でも使えますが、そこではタブは書けません。`@@` / `!!` と同じく `+` の明示グループには適用されません。長い領域は次の raw mode で囲みます（領域内はタブと `!text{...}` を含む完全な verbatim です）。
...
※ これらの規則は、環境の本文に書く行にも同じように適用されます。たとえば `\item まとめ:` は、次の行が4スペース以上字下げされていなければそのまま生の TeX ですが、直後に4スペース字下げしたブロックが続くと行末のコロンが suite を開こうとしてパースエラーになります（その場合は `!| \item まとめ:` または `\item まとめ:{}` と書きます）。詳細は [doc/dsl.md 8 章](doc/dsl.md) の表を参照してください。

---

### 2. 2つのブロック形式（Suite Suffix）

行末のコロンの数で、インデントされた子要素の受け渡し方を指定します。コマンド行の単独の `:` は、直後にインデントされたブロックが続くときだけ suite として読まれます。続かなければ行末の `:` は本文の一部です（`\textbf{Note}:` はそのまま出力されます）。
...
**A. 一切問題ありません。**  
TeXFlux が認識するのは、行頭のプレフィックス（`@`, `!`, `\`）や行末の suite suffix（`:` / `::`）、前後をスペースで区切られた `>>` だけです。数式の中（`$f: X \to Y$` など）やグループ内の文字を勝手にパースして破壊することはありません。コマンド行の末尾の `:` も、直後に4スペース字下げしたブロックが続かなければ本文の一部としてそのまま透過されます。
