...
※ 行頭の `@` / `!` を1行だけ生の TeX として出力するには `@@` / `!!` と書きます（`!!foo` → `!foo`、`!!!foo` → `!!foo`）。環境本文や生成シーケンスの payload（`- !!bar`）でも使えます。`+` の明示グループは内容全体が raw TeX なので、この escape は適用されません。長い領域は次の raw mode で囲みます（領域内はタブと `!text{...}` を含む完全な verbatim です）。
...
※ これらの規則は、環境の本文に書く行にも同じように適用されます。たとえば `\item まとめ:` は行末のコロンが予約されているためパースエラーになります（`\item まとめ:{}` と書きます）。詳細は [doc/dsl.md 8 章](doc/dsl.md) の表を参照してください。
