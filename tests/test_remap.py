...
def _write_map(
    ...
    *,
    generated: bytes,
    source_files: dict[int, tuple[str, bytes]],
...
class SyncTeXRemapTests(unittest.TestCase):
    ...
    def test_hash_mismatch_is_a_hard_error(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            (root / "out.tex").write_bytes(b"stale\n")

            with self.assertRaisesRegex(RemapError, "generated file hash"):
                remap_synctex(
                    _sync(b"v1,1:100,200:3,4,5"),
                    map_paths=[map_path],
                    synctex_path=root / "out.synctex",
                )
    ...
    def test_source_hash_mismatch_and_unmatched_generated_input_are_hard_errors(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            (root / "source.tfx").write_bytes(b"changed\n")
            ...
            (root / "source.tfx").write_bytes(b"source\n")
    ...
    def test_existing_source_tags_are_reused_and_new_tags_are_deterministic(self):
        ...
        self.assertIn(f"Input:4:{root / 'new.tfx'}\n".encode(), rewritten)
    ...
    def test_duplicate_maps_for_one_generated_input_are_rejected(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            first = _write_map(root, **kwargs)
            second = _write_map(root, **kwargs, name="second.tfxmap")
    ...
    def test_output_is_atomic_and_in_place_replacement_preserves_mode(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(list(root.glob(".out.synctex.*.tmp")), [])
    ...
    def test_cli_supports_repeatable_map_and_output(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            output_path = root / "result.synctex"
    ...
    def test_gzip_remap_preserves_the_container(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            rewritten = remap_synctex(
                compressed,
                map_paths=[map_path],
                synctex_path=root / "out.synctex.gz",
            )
