...
from beamercraft.cli import main
...
class CliTests(unittest.TestCase):
    ...
    def test_compile_failure_does_not_change_existing_output(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("bad.bmc:1:6: parse error:", stderr.getvalue())
    ...
    def test_same_path_and_missing_parent_are_rejected(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            stderr = StringIO()
            with contextlib.redirect_stderr(stderr):
                self.assertEqual(main([str(input_path), "-o", str(input_path)]), 1)
            self.assertIn("different paths", stderr.getvalue())

            output_path = root / "missing" / "out.tex"
            with contextlib.redirect_stderr(StringIO()):
                self.assertEqual(main([str(input_path), "-o", str(output_path)]), 1)
